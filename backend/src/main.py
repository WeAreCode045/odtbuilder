...
def clean_html_text(raw_html):
    """
    Simple function to strip HTML tags for now to ensure clean text output.
    Replaces <br> and <p> boundaries with newlines if needed, then strips tags.
    """
    ...
    # Decode basic entities if any (odfpy handles XML escaping, but we might have &nbsp;)
...
def process_node(node_id, craft_data, parent_element, doc):
    ...
    # --- TEKST (Paragraph) ---
        ...
        # Clean HTML tags to avoid outputting raw HTML in ODT
        clean_text = clean_html_text(raw_text)
        
        # Split by newline to handle breaks properly
        lines = clean_text.split('\n')
        for i, line in enumerate(lines):
            if i > 0:
                # We could add line breaks, but usually new paragraph is better logic or LineBreak
                # For a single Tekst block, let's treat it as one P but with LineBreaks if needed
                # However, simpler to just start a new P if it was a real block break, 
                # but here we are inside one Node.
                # Let's use LineBreak()
                p.addElement(LineBreak())
            
            # Since we construct P outside loop, wait...
            pass
    ...
    # --- AFBEELDING ---
        ...
        add_style(doc, style_name, "paragraph", paragraph_props={"textalign": align, "marginbottom": "0.5cm"})
        p = P(stylename=style_name)
        p.addText("[AFBEELDING PLACEHOLDER]")
        parent_element.addElement(p)

    # --- SCRIPT (Custom Script Element) ---
    ...
    # --- COLUMN (Standalone) ---
        ...
        # If a column exists outside a row
        for child_id in children_ids:
            process_node(child_id, craft_data, parent_element, doc)
